...
public class SalesmanDao extends DAO<Salesman>{
	...
	public void create(Salesman element) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(CREATE_QUERY);) {
            ...
            if (ex.getMessage().contains("not-null")) {
                throw new SQLException("Erro ao inserir vendedor: pelo menos um campo est� em branco.");
            } else {
        ...
        try (PreparedStatement statement = connection.prepareStatement(READ_QUERY)) {
            ...
            try (ResultSet result = statement.executeQuery()) {
                if (result.next()) {
                    ...
                    throw new SQLException("Erro ao visualizar: vendedor n�o encontrado.");
                }
            }
        } catch (SQLException ex) {
            System.err.println(ex.getMessage());

            if (ex.getMessage().equals("Erro ao visualizar: vendedor n�o encontrado.")) {
                throw ex;
            } else {
	...
	public void update(Salesman element) throws SQLException {
		 try (PreparedStatement statement = connection.prepareStatement(UPDATE_QUERY)) {
	            ...
	            if (statement.executeUpdate() < 1) {
	                throw new SQLException("Erro ao editar: vendedor n�o encontrado.");
	            }
	            
	        } catch (SQLException ex) {
	            System.err.println(ex.getMessage());

	            if (ex.getMessage().equals("Erro ao editar: vendedor n�o encontrado.")) {
	                throw ex;
	            } else if (ex.getMessage().contains("not-null")) {
	                throw new SQLException("Erro ao editar vendedor: pelo menos um campo est� em branco.");
	            } else {
	...
	public void delete(Salesman element) throws SQLException {
	       try (PreparedStatement statement = connection.prepareStatement(DELETE_QUERY)) {
	            ...
	            if (statement.executeUpdate() < 1) {
	                throw new SQLException("Erro ao excluir: vendedor n�o encontrado.");
	            }
	        } catch (SQLException ex) {
	            System.err.println(ex.getMessage());

	            if (ex.getMessage().equals("Erro ao excluir: vendedor n�o encontrado.")) {
	                throw ex;
	            } else {
